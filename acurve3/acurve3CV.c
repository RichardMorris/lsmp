...
acurve3()
{
	...
	expand(temp_eqn2);
                ...
                e,rpe_names[0],rpe_names[1],rpe_names[2]) )
        {
                ...
                initoogl(colour);
                set_acurve3_sigs();
                flag = marmain(a,b,c,d,e,xmin,xmax,ymin,ymax,zmin,zmax);
                reset_acurve3_sigs();
	...
	print_jvx_header("acurve3","Automatically generated algebraic curve in 3D");
        print_eqn(main_eqn1); printf(";\n");
        print_eqn(main_eqn2); printf(";\n");
        for(i=0;i < n_params;++i)
                printf(" %s = %f;\n",rpe_names[i+n_vars],rpe_vals[i+n_vars]);
	printf(" %s = [%f,%f];\n%s = [%f,%f];\n%s = [%f,%f];\n",
		rpe_names[0],xmin,xmax,rpe_names[1],ymin,ymax,
        ...
        printf(" _faces = %d;\n",LINK_FACE_LEVEL);
        printf(" _edges = %d;\n",SUPER_FINE);
	print_jvx_header2("algebraic curve in 3D");
