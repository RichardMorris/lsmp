...
PsurfMapTgt(HPoint3 *pl,HPoint3 *tgt)
{
	float val,u1,u2;
...
int_curve(char *filename)
{
	...
	if( global_mode == MODE_PSURF || global_mode == MODE_PSURF_PROJ )
	{
		return(int_curve_psurf(filename));
	}
	...
	else
	{
		...
		vert_per_vect = OOGLNewN(short,num_pts*2);
		coltab = OOGLNewN(short,num_pts*2);
		n_vect = num_pts * 2;
	}
...
read_def(FILE *fp)
{
	int     dimension,normals,colours,fourD_eqns,local_oriented;
	char	str[100];
