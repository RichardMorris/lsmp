...
void get_existing_faces(box_info *box)
{
...
void create_new_faces(box_info *box)
{
	if( box->ll == NULL )
	{
		box->ll = alloc_face();
...
void get_existing_edges(box_info *box,face_info *face,int code)
{
	box_info *adjacient_box;
...
void create_new_edges(face_info *face)
{
	if( face->x_low == NULL )
	{
...
int get_sols_on_edge(edge_info *edge,sol_info *sols[2],int count)
{
...
int count_sols_on_edge(edge_info *edge)
{
	int count = 0;
...
sol_info *get_nth_sol_on_edge(edge_info *edge,int n)
{
	sol_info *temp;
...
int get_sols_on_face(face_info *face,sol_info *sols[2])
{
	int count;
...
int count_sols_on_face(face_info *face)
{
	int count;
...
sol_info *get_nth_sol_on_face(face_info *face,int n)
{
	sol_info *temp;
...
int get_nodes_on_face(face_info *face,node_info *nodes[2],int count)
{
	node_info *temp;
...
int count_nodes_on_face(face_info *face)
{
	int count=0;
	node_info *temp;
...
int get_nodes_on_box_faces(box_info *box,node_info *nodes[2])
{
	int count;
...
node_info *get_nth_node_on_box(box_info *box,int n)
{
	node_info *temp;
...
void split_face(face_info *face,face_info *face1,face_info *face2,
			...
			face_info *face3,face_info *face4)
{
	...
	{
	    node = face->nodes;
fprintf(stderr,"distribute_nodes:\n");
print_face(face);
print_node(node);
		...
		break;
	default:
		fprintf(stderr,"distribute_nodes: bad type %d\n",face->type);
	    }  /* end switch */
	} /* end while */
}
...
void split_box(box_info *box,box_info *lfd,box_info *rfd,box_info *lbd,box_info *rbd,
