...
report_error(int code,char *str,int errcode)
{
...
report_error2(int code,char *format,char *str,int errcode)
{
                printf("Content-type: text/plain%c%c",10,10);
                if(code == HEAD_ERROR)
                {
...
;
       ...
       temp_file = fopen(arg_filename,"w");
        ...
        {
                report_error2(HEAD_ERROR,"Sorry your value for the timeout (%s) 
exceeds the allowable maximum.",timeout_ent,13);
                exit(1);
        }
#endif
...
acurve()
{
	...
	{
		fprintf(stderr,"Can't calculate curve  - bad equations\n");
		return;
	}
