...
void set_input_functions(funlist)
eqn_funs *funlist;
{
	base_funs = funlist;
}
...
va_dcl
{
	...
	if( eqn == NULL )
	{
		eprintf("NULL profile for external function\n");
		return(funlist);
	}
	if( eqn->op != '*' || eqn->u.n.l->op != NAME)
	{
		eprintf("Bad profile for external function\n");
		eprintf("%s\n",profile);
		return(funlist);
	}
	...
	new->name = strdup(eqn->u.n.l->u.str);
...
eqnode  *eqn;
{
	...
	if( eqn == NULL )
	{
		eprintf("Tried to add a NULL internal function\n");
		return(funlist);
	}
	if( eqn->op != '=' || eqn->u.n.l->op != '*'
		|| eqn->u.n.l->u.n.l->op != NAME)
	{
		eprintf("Bad syntax for internal function equation\n");
		eprint_eqn(eqn);
		return(funlist);
	}
	new = grballoc(eqn_funs);
	...
	new->name = strdup(eqn->u.n.l->u.n.l->u.str);
	...
	for(i=0;i<new->nvars;++i)
	{
		...
		{
			new->vars[i]=strdup(args->u.n.l->u.str);
			args = args->u.n.r;
		}
#endif
		else
...
/*
 * Function:	add_sum_function
 * Action;	adds a sum-type function to a list
 */

eqn_funs *add_sum_fun(eqn_funs *funlist,char *profile,char *begin_eqn,char *itt_eqn)
{
	eqn_funs *new;
	int	i;
	eqnode *eqn,*args;
	...
	if( eqn == NULL )
	{
		eprintf("NULL profile for sum function\n");
		return(funlist);
	}
	if( eqn->op != '*' || eqn->u.n.l->op != NAME)
	{
	...
	/* Now construct the itteration rpe */
	...
	new->eqn = sscan_eqn(itt_eqn);
	if(new->eqn == NULL)
	{
		eprintf(stderr,"NULL equation in add_sum_fun\n");
	}
	use_functions(new->eqn,funlist);
	new->rpe = make_rpe(new->eqn,4,itt_names);
/*
printf("eqn "); print_eqn(new->eqn); printf("\n");
fprint_rpe(stdout,new->rpe,itt_names);
*/

	new->eqn2 = sscan_eqn(begin_eqn);
	if(new->eqn2 == NULL)
	{
		eprintf(stderr,"NULL equation in add_sum_fun\n");
	}
	use_functions(new->eqn2,funlist);
	new->rpe2 = make_rpe(new->eqn2,4,itt_names);
/*
printf("eqn2 "); print_eqn(new->eqn2); printf("\n");
fprint_rpe(stdout,new->rpe2,itt_names);
*/
...
va_dcl
{
	...
	if( eqn == NULL )
	{
		eprintf("NULL profile for external mapping\n");
		return(funlist);
	}
	if( eqn->op != '*' || eqn->u.n.l->op != NAME)
	{
		eprintf("Bad profile for external mapping\n");
		eprintf("%s\n",profile);
		return(funlist);
	}
	new = grballoc(eqn_funs);
...
eqnode  *eqn;
{
	...
	if( eqn == NULL )
	{
		eprintf("Tried to add a NULL internal mapping\n");
		return(funlist);
	}
	if( eqn->op != '=' || eqn->u.n.l->op != '*'
		|| eqn->u.n.l->u.n.l->op != NAME)
	{
		eprintf("Bad syntax for internal mapping equation\n");
		eprint_eqn(eqn);
		return(funlist);
	}
	new = grballoc(eqn_funs);
...
eqn_funs *funs;
{
	...
	while(funs != NULL )
	{
	    ...
	    case SUM_FUN:
		fprintf(fp,"SUM_FUN %s(",funs->name);
...
eqn_funs *funs;
{
	...
	while(funs != NULL )
	{
	    ...
	    case SUM_FUN:
		eprintf("SUM_FUN %s(",funs->name);
		for(i=0;i<funs->nvars;++i)
		{
			if(i>0) eprintf(" , ");
			eprintf("%s",funs->vars[i]);
		}
		eprintf(")\t");
		eprint_eqn(funs->eqn);
		eprintf(";  ");
		eprint_eqn(funs->eqn2);
		eprintf(";\n");
		break;


	    case EXTERN_FUN:
		eprintf("FUNCTION %s(",funs->name);
		for(i=0;i<funs->nvars;++i)
		{
			if(i>0) eprintf(" , ");
			eprintf("%s",funs->vars[i]);
		...
		eprintf(")\t");
		eprintf("defined internally\n");
		for(i=0;i<funs->nvars;++i)
		{
			eprintf("\td%s_d%s = ",funs->name,funs->vars[i]);
			eprint_eqn(funs->diff[i]);
			eprintf(";\n");
		}
		break;
		...
		eprintf(")\t");
		eprintf("dimension %d defined internally\n",funs->dim);
		for(i=0;i<funs->nvars;++i)
		{
			eprintf("\td%s_d%s = ",funs->name,funs->vars[i]);
			eprint_eqn(funs->diff[i]);
	    ...
	    default:
		eprintf("Unknow function type %d\n",funs->type);
		break;
	    }
	    funs = funs->next;
	}
...
eqn_funs *funlist;
{
	...
	if( eqn == NULL )
	{
		eprintf("NULL equation in use_functions\n");
		return(FALSE);
	}
#ifdef PRI_USE_FUNS
	eprintf("use_functions: eqn\n");
	eprint_eqn(eqn);
	fprintf("\n");
#endif
	...
	else if( eqn->op == NAME )
	{
		...
		while(fun != NULL)
		{
			...
			else if( !strcmp(eqn->u.str,fun->name) )
			{
				eprintf("Function %s found, but not in correct context\n",fun->name);
			}
			fun = fun->next;
		}
		return(TRUE);
...
eqn_funs *funlist;
{
	...
	funlist = add_external_function(funlist,"tanh(x)",tanh,
		"1-(tanh(x))^2");
#ifndef __BORLANDC__
#ifndef GCC_COMP
/*
...
#ifdef SUM_OPS	
	funlist = add_sum_fun(funlist,"Sum(f,x,l,h,inc)","val","res+val");
	funlist = add_sum_fun(funlist,"Prod(f,x,l,h,inc)","val","res*val");
	funlist = add_sum_fun(funlist,"Min(f,x,l,h,inc)","val","min(res,val)");
	funlist = add_sum_fun(funlist,"Max(f,x,l,h,inc)","val","max(res,val)");
	funlist = add_sum_fun(funlist,"Mean(f,x,l,h,inc)","val",
		"((num-1)*res+val)/num");
#endif
