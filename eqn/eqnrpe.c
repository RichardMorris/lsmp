...
#define rpe_sum_max 32
#define rpe_const_base (rpe_fun_base + rpe_fun_max + rpe_sum_max)
#ifdef BOR_THINK
#define rpe_const_max  1000
#else
...
rpeint *rpe_sums[rpe_sum_max];
eqn_funs *rpe_sum_funs[rpe_sum_max];
#define rpe_sum_base (rpe_fun_base+rpe_fun_max)
int rpe_sum_ptr = 0;
...
eqnode *eqn; int n; char **names;
{
  ...
  if(eqn == NULL )
  {
	eprintf("make_rpe: NULL eqnation\n");
	return(NULL);
  }

  if(names == NULL)
  {
	eprintf("make_rpe: NULL names list\n");
	return(NULL);
  }
  for(i=0;i<n;++i)
  {
	if(names[i] == NULL)
	{
		eprintf("make_rpe: NULL name number %d\n",i);
		return(NULL);
	}
  }
...
rpeint rpe[]; eqnode *eqn; int n; char **names;
{
  ...
  case NAME:
    ...
    eprintf("make_rpe2: name not found %s\n",eqn->u.str);
    return( FALSE );
  ...
  case NUMBER:
    ...
    {
      eprintf("make_rpe2: too many constants\n");
      return(FALSE);
    }
    return( TRUE );
  ...
  case FUNCTION:
	...
	case SUM_FUN:
		...
		rpe_sums[rpe_sum_ptr] =
				make_rpe(eqn->u.f.a->u.n.l,n,names);
		rpe_sum_funs[rpe_sum_ptr] = eqn->u.f.f;
		leftres = make_rpe2(rpe,eqn->u.f.a->u.n.r, n, names );

		rpe[rpe_ptr++] = (rpeint)
			 rpe_sum_ptr + rpe_sum_base;

		++rpe_sum_ptr;
		if( rpe_sum_ptr >= rpe_sum_max )
		{
	...
	case INTERN_MAP: case EXTERN_MAP:
		...
		{
			eprintf("make_rpe2: too many functions\n");
			return(FALSE);
		}
		return(leftres);
	default:
		eprintf("Bad function type %d in make_rpe\n",eqn->u.f.f->type);
		return(FALSE);
	}

  default:
	eprintf("make_rpe2: bad op ");
        eprint_op(eqn->op);
        eprintf("\n");
	break;
  }
...
rpeint rpe[]; char **names;
{
  ...
  while( c != END_RPE && c != rpe_op_base + '=' )
  {
    if( c < rpe_op_base )          eprintf("\tvar\t%s\n",names[c]);
    else if ( c == rpe_op_base + INT_POW )
				   eprintf("\top\t^ %d\n",rpe[ptr++]);
    else if ( c < rpe_fun_base )   eprintf("\top\t%c\n",c-rpe_op_base);
    else if ( c < rpe_const_base ) eprintf("\tfun\t%s\n",
					rpe_funs[c-rpe_fun_base]->name);
...
register double *vars;
{
  ...
  if( rpe == NULL )
  {
    eprintf("eval_rpe: NULL reverse polish string\n");
    return(sqrt(-1.0));
  }
...
#define StackERR {\
      if( stackptr >= stack_size ) { \
        eprintf("check_rpe: stack full, require %d\n",stackptr); \
...
char **names;
{
  ...
  if( rpe == NULL )
  {
    eprintf("check_rpe: NULL reverse polish string\n");
    return(FALSE);
  }
  ...
  do
  {
    ...
    if( c < rpe_op_base )          /* c a variable */
    {
      ...
      if( c >= n )
      {
        eprintf("check_rpe: bad variable referance var[%d]\n",c);
        flag =  FALSE;
      }
    }
	...
	break;
      ...
      default:
	eprintf("Bad type for function when checking rpe %d\n",
		fun->type);
	break;
      }
    }
    ...
    else			/*  c a constant */
    {
      ...
      {
        eprintf("check_rpe: bad constant ref, const[%d]\n",c-rpe_const_base);
        flag =  FALSE;
      }
    }    /* end if */
  ...
  {
	eprintf("check_rpe: stack corupted on exit: initial %d final %d\n",
		initStackPtr,stackptr);
	eprint_rpe(rpe,names);
/*
  	flag = FALSE;
*/
...
void clear_rpe_const() {rpe_const_ptr = 0;}
