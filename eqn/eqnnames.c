 ...
 * Action:	returns the index of name, 0 if name is not found
 *		and -1 on error
 */

int get_name_index(ptr,name)
eqn_names *ptr; char *name;
{
	int pos;
	if(name == NULL)
	{
		eprintf("NULL name while trying to find the index\n");
		return(-1);
	}
	pos = 1;
	while( ptr != NULL )
