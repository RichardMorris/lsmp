...
eqnode *base,*sub;
{
  ...
  if( sub == NULL )
  {
	eprintf("substitute: sub eqn is NULL\n");
	return(FALSE);
  }
  if( base == NULL )
  {
	eprintf("substitute: base eqn is NULL\n");
	return(FALSE);
  }
  if( sub->op != '=' )
  {
	eprintf("substitute: sub not an assignment\n");
	eprint_eqn(sub);
	eprintf("\n");
    return(FALSE);
  }
  ...
  {
	eprintf("Bad substitution equation\n");
        eprint_eqn(sub);
	eprintf("\n");
	return(FALSE);
  }
  ...
  else if( sub->u.n.l->op == '*' && sub->u.n.l->u.n.l->op == NAME )
  {
		...
		&& !strcmp(base->u.n.l->u.str,sub->u.n.l->u.n.l->u.str ) )
	{
		...
		{
			eprintf("Different number of arguments in profile and actual occurance\n");
			eprintf("Profile: ");
			eprint_eqn(sub->u.n.l);
			eprintf("\nActual: ");
			eprint_eqn(base);
			return(FALSE);
		}
  ...
  {
    eprintf("substitute: bad substitution equation:\n");
    eprint_eqn(sub);
    eprintf("\n");
    return(FALSE);
  }
