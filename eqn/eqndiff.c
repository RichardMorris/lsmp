...
char *var_name;
{
	...
	/* should really be a^b -> d(a^b)_da da_dx + d(a^b)_db db_dx */
	    ...
	    {
		eprintf("Different counts while differentiating '^' %d %d\n",leftcount,rightcount);
		diff_wrt(base->u.n.l,var_name);
		diff_wrt(base->u.n.r,var_name);
		break;
	    }
	...
	default:
		eprintf("diff_wrt couldn't handel code %d\n",base->op);
	} /* end switch */
...
char *name;
{
	...
	{
		eprintf("Don't know how to differentiate a sum type function %s\n",
			base->u.f.f->name);
		return;
	}
...
eqnode *eqn;
{
	...
	{
		eprintf("Must have 'eqn,name' for differentiation\n");
		eprint_eqn(eqn);
		eprintf("\n");
		return(eqn);
	}
