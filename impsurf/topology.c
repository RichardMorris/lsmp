...
face_info *face;
{
	if( face->x_low == NULL )
	{
		face->x_low = allocedge();
		make_face_edge(face,X_LOW,face->x_low);
	}
	if( face->x_high == NULL )
	{
		face->x_high = allocedge();
		make_face_edge(face,X_HIGH,face->x_high);
	}
	if( face->y_low == NULL )
	{
		face->y_low = allocedge();
		make_face_edge(face,Y_LOW,face->y_low);
	}
	if( face->y_high == NULL )
	{
		face->y_high = allocedge();
		make_face_edge(face,Y_HIGH,face->y_high);
	}

	/* Now work with verticies */

	if( face->x_low->low == NULL )
	{
		if( face->y_low->low == NULL )
		{
			face->y_low->low = 
			face->x_low->low = allocvert();
			make_face_vert(face,X_LOW_Y_LOW,face->x_low->low);
		}
		face->x_low->low = face->y_low->low;
	}
	else
	{
		if( face->y_low->low == NULL )
			face->y_low->low = face->x_low->low;

		else if (face->x_low->low != face->y_low->low )
		{
			fprintf(stderr,"Two ref %p %p to \n",
				face->x_low->low, face->y_low->low);
			printvert(face->x_low->low);
		}
	}

	if( face->x_low->high == NULL )
	{
		if( face->y_high->low == NULL )
		{
			face->y_high->low = 
			face->x_low->high = allocvert();
			make_face_vert(face,X_LOW_Y_HIGH,face->x_low->high);
		}
		face->x_low->high = face->y_high->low;
	}
	else
	{
		if( face->y_high->low == NULL )
			face->y_high->low = face->x_low->high;

		else if (face->x_low->high != face->y_high->low )
		{
			fprintf(stderr,"Two ref %p %p to \n",
				face->x_low->high, face->y_high->low);
			printvert(face->x_low->high);
		}
	}

	if( face->x_high->high == NULL )
	{
		if( face->y_high->high == NULL )
		{
			face->y_high->high = 
			face->x_high->high = allocvert();
			make_face_vert(face,X_HIGH_Y_HIGH,face->x_high->high);
		}
		face->x_high->high = face->y_high->high;
	}
	else
	{
		if( face->y_high->high == NULL )
			face->y_high->high = face->x_high->high;

		else if (face->x_high->high != face->y_high->high )
		{
			fprintf(stderr,"Two ref %p %p to \n",
				face->x_high->high, face->y_high->high);
			printvert(face->x_high->high);
		}
	}

	if( face->x_high->low == NULL )
	{
		if( face->y_low->high == NULL )
		{
			face->y_low->high = 
			face->x_high->low = allocvert();
			make_face_vert(face,X_HIGH_Y_LOW,face->x_high->low);
		}
		face->x_high->low = face->y_low->high;
	}
	else
	{
		if( face->y_low->high == NULL )
			face->y_low->high = face->x_high->low;

		else if (face->x_high->low != face->y_low->high )
		{
			fprintf(stderr,"Two ref %p %p to \n",
				face->x_high->low, face->y_low->high);
			printvert(face->x_high->low);
		}
	}
...
face_info *face,*face1,*face2,*face3,*face4;
{
	...
	face1->x_high = face2->x_low = allocedge();
	make_face_edge(face1,X_HIGH,face1->x_high);

	face3->x_high = face4->x_low = allocedge();
	make_face_edge(face3,X_HIGH,face3->x_high);

	face1->y_high = face3->y_low = allocedge();
	make_face_edge(face1,Y_HIGH,face1->y_high);

	face2->y_high = face4->y_low = allocedge();
	make_face_edge(face2,Y_HIGH,face2->y_high);
	create_new_edges(face1);
	create_new_edges(face2);
	create_new_edges(face3);
	create_new_edges(face4);
}
